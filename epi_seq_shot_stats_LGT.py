import argparse
import os
import sys
import time
import tqauth
import json
from datetime import datetime
import tractor.api.query as tq
sys.path.extend(['/sw/pipeline/rendering/renderfarm-tools',
                 '/sw/pipeline/rendering/renderfarm-reporting'])
from tractor_utils import job_task_utils as jtu
from reporting_tools import reporting_tools as rt
from sg_utils import sg_utils

tq.setEngineClientParam(user=tqauth.USERNAME, password=tqauth.PASSWORD)

# User Input and Globals
PARSER = argparse.ArgumentParser(description="Get LGT TPF per Show/Episode/Shot. "
                                             "Example: get_tpf -show TUP -epi A104 -v")
PARSER.add_argument("-s", "--show", type=str, help="Show to grab the TPF for. "
                                                   "Example: '-show TUP'")

PARSER.add_argument("-e", "--episode", type=str, help="Episode to grab the TPF for. "
                                                      "Example: '-epi A104'")
PARSER.add_argument("-sh", "--shot", type=str, help="Shot to grab the TPF for. "
                                                    "Example: '-shot B103_CE1890'")

PARSER.add_argument('-epi', action='store_true', help="Show Episode Breakdown")
PARSER.add_argument('-seq', action='store_true', help="Show Sequence Breakdown")
PARSER.add_argument('-shot', action='store_true', help="Show Shot Breakdown")
PARSER.add_argument('-seq_shot', action='store_true', help="Seq and Shot Breakdown")
PARSER.add_argument('-all_breakdown', action='store_true', help="Epi, Seq and Shot Breakdown")

PARSER.add_argument('-all_epi', action='store_true', help="Grab stats for all Episodes")
PARSER.add_argument('-daily_epi', action='store_true', help="Grab stats for Episodes that rendered in last 24 hours")

ARGS = PARSER.parse_args()
SHOW = ARGS.show.upper()
EPISODE = ARGS.episode
SHOT = ARGS.shot

if ARGS.epi:
    BREAKDOWNS = ['epi']
elif ARGS.seq:
    BREAKDOWNS = ['seq']
elif ARGS.shot:
    BREAKDOWNS = ['shot']
elif ARGS.seq_shot:
    BREAKDOWNS = ['seq', 'shot']
elif ARGS.all_breakdown:
    BREAKDOWNS = ['epi', 'seq', 'shot']

DAILY_EPISODES = bool(ARGS.daily_epi)
ALL_EPISODES = bool(ARGS.all_epi)

TODAY = datetime.today().strftime('%m%d%Y')
TIME_NOW = datetime.now()
TIME = TIME_NOW.strftime('%Y-%m-%d %H:%M:%S')
FILE_DIR = "/mnt/w/PubLogs/render/datadog_epi_seq_stats_{}.log".format(TODAY)
# FILE_DIR = "datadog_epi_seq_stats_{}.log".format(TODAY)

with open('/sw/tractor/reporting_data/cg_keyshots.json') as json_file:
    KEYSHOTS = json.load(json_file)
with open('/sw/tractor/reporting_data/cg_parentshots.json') as json_file:
    PARENTSHOTS = json.load(json_file)

IGNORE_EPI = []
if SHOW == "LMA":
    IGNORE_EPI = ['R001']
elif SHOW == "MLP":
    IGNORE_EPI = ['C001', 'E999', 'M001', 'M002', 'T001', 'T002']
elif SHOW == "PWP":
    IGNORE_EPI = ['C101', 'M101', 'N101', '0101', 'O101', 'R101', 'R102',
                  'S101', 'S102', 'S103', 'S104', 'S105', 'S106', 'S107',
                  'S108', 'S109', 'S110', 'S111', 'S112', 'S113', 'S114',
                  'S115', 'S116', 'S117', 'S118', 'T101', 'T102']
elif SHOW == "TUP":
    IGNORE_EPI = ['E800', 'E801', 'E901', 'E991', 'E993', 'E995', 'M002',
                  'M003', 'S001', 'S002', 'S003', 'S004', 'S005', 'S006',
                  'S007', 'S008', 'S009', 'T001', 'T006', 'T007']
elif SHOW == "ZOM":
    IGNORE_EPI = ['P001', 'R001', 'R002', 'R003', 'R004', 'R005']

# Setting the shows season
if SHOW == "TUP":
    SEASON = "TUP002"
elif SHOW == "LMA":
    SEASON = "SST003"
else:
    SEASON = "{}001".format(SHOW)


def get_episodes_rendered():
    """Get the episode names that have rendered for the show"""

    tq_addition = ""
    # If we just want to grab the episodes rendered in the last 24 hours
    if DAILY_EPISODES:
        tq_addition = "and stoptime > -24h"

    # 24 hour episode search
    jobs_rendered = tq.jobs("done and projects like {0} and title like LGT.v "
                            "and title like katana and title like {1} {2}"
                            "".format(SHOW, SEASON, tq_addition),
                            limit=0,
                            archive=True)

    # We just want the unique epiosde names.
    episodes_rendered = set()
    for job in jobs_rendered:
        episode = jtu.get_episode(job['title'])
        episodes_rendered.add(episode)

    return list(episodes_rendered)


def datadog_print(breakdown, stats_dict, prod_order):

    def create_datadog_dict(sdict):

        out_dict = {
            'timestamp': TIME,
            'show': SHOW,
            'shots': sdict['shots'],
            'frames': sdict['frames'],
            'ffr_jobs': round(sdict['ffr'], 2),
            'fml_jobs': round(sdict['fml'], 2),
            'passes': round(sdict['passes'], 2),
            'iterations': round(sdict['iter'], 2),
            'ctpt_stand': int(sdict['ctpt'] / 36),
            'ctpf_stand': int(sdict['ctpf'] / 36),
            'chps': round((sdict['chps'] / 3600.0), 2),
            'max_rss': round(sdict['mrss'], 2)}

        return out_dict


    log_file = open(FILE_DIR, "a")
    for epi in stats_dict:
        p_order = prod_order.get(epi, 99)

        if breakdown == 'epi':
            if epi in IGNORE_EPI:
                continue
            elif stats_dict[epi]['shots'] < 3:
                continue
            out_dict = create_datadog_dict(stats_dict[epi])
            out_dict['breakdown_type'] = 'episode'
            out_dict['episode'] = epi
            out_dict['prod_order'] = p_order
            log_file.write("{}\n".format(json.dumps(out_dict)))

        elif breakdown == 'seq':
            if epi in IGNORE_EPI:
                continue
            for seq in stats_dict[epi]['seq_list']:
                out_dict = create_datadog_dict(stats_dict[epi][seq])
                out_dict['breakdown_type'] = 'sequence'
                out_dict['episode'] = epi
                out_dict['sequence'] = seq
                out_dict['prod_order'] = p_order
                log_file.write("{}\n".format(json.dumps(out_dict)))

        elif breakdown == 'shot':
            for seq in stats_dict[epi]['seq_list']:
                for shot in stats_dict[epi][seq]['shot_list']:
                    stats_dict[epi][seq][shot]['shots'] = 1
                    out_dict = create_datadog_dict(stats_dict[epi][seq][shot])
                    out_dict['breakdown_type'] = 'shot'
                    out_dict['episode'] = epi
                    out_dict['sequence'] = seq
                    out_dict['shot'] = shot
                    out_dict['prod_order'] = p_order
                    out_dict['artist'] = stats_dict[epi][seq][shot]['artist']
                    out_dict['shot_tag'] = '-'
                    if shot in KEYSHOTS[SHOW]:
                        out_dict['shot_tag'] = 'Key'
                    elif shot in PARENTSHOTS[SHOW]:
                        out_dict['shot_tag'] = 'Parent'
                    log_file.write("{}\n".format(json.dumps(out_dict)))

def get_shot_assets(sg):

    filters = [["project.Project.tank_name", "is", SHOW]]

    all_shots = sg.find("Shot", filters, fields=['assets', 'code'],
                        order=[{"direction": "asc", "field_name": "id"}])
    all_assets = sg.find("Asset", filters, fields=['code', 'sg_asset_type'],
                         order=[{"direction": "asc", "field_name": "id"}])

    all_assets_dict = {asset['id']: asset for asset in all_assets}

    shot_asset_dict = {}
    for shot in all_shots:
        shot_asset_dict[shot['code']] = {'num_assets': 0}
        sass_dict = shot_asset_dict[shot['code']]
        shot['assets'] = [all_assets_dict[asset['id']] for asset in shot['assets']]
        for asset in shot['assets']:
            sass_dict['num_assets'] += 1
            if asset['sg_asset_type'] in sass_dict.keys():
                sass_dict[asset['sg_asset_type']] += 1
            else:
                sass_dict[asset['sg_asset_type']] = 1

    return shot_asset_dict

def get_production_order(sg):

    filters = [["project.Project.tank_name", "is", SHOW]]
    fields = ['code', 'sg_production_order']

    sg_epi_dict = sg.find("Episode", filters, fields)

    prod_epi_order = {}
    for epi_dict in sg_epi_dict:
        episode = epi_dict['code'][-4:]
        prod_order = epi_dict['sg_production_order']
        if not prod_order:
            prod_order = 99

        prod_epi_order[episode] = prod_order

    return prod_epi_order


def add_comment(new_jobs):

    num_jobs = len(new_jobs)
    i = 0
    for job in new_jobs:
        # time.sleep(.5)
        i += 1
         # If job was archived, unarchive it so we can write the comment
        if job['deletetime']:
            tq.undelete('jid={}'.format(job['jid']))
            time.sleep(1)

        new_comm = "EpiShotStats | {}".format(job['comment'])
        tq.jattr('jid={}'.format(job['jid']), key='comment', value=new_comm)


def get_episode_data(epi, dept):

    epi_name = epi.split("_")[0]
    epi_stats_log = ("/sw/pipeline/rendering/renderfarm-reporting"
                     "/tq_data/epi_shot_stats/"
                     "{0}/{1}/{2}_{3}_{1}_stats.log"
                     "").format(SHOW, dept, SEASON, epi_name)
    # Stats Directory
    stats_dir = epi_stats_log.replace(epi_stats_log.split('/')[-1], '')

    # Read Old Data
    if os.path.exists(epi_stats_log):
        with open(epi_stats_log, 'r') as f_obj:
            epi_data = json.loads(f_obj.read())
    else:
        if not os.path.isdir(stats_dir):
            os.makedirs(stats_dir)
        epi_data = []

    # Find New Data from Tractor
    if dept == "LGT":
        job_tq = ("Job.title like LGT.v and Job.title like katana "
                  "and Job.title not like Groom_QC")
        task_tq = "Task.title like Katana_Render"

    elif dept == "ANM":
        job_tq = "Job.title like 'QC Render'"
        task_tq = "Task.title like 'Render Katana'"

    tq_search = ("projects like {} and "
                 "Job.comment not like EpiShotStats and "
                 "Job.title like _{} and "
                 "Job.numdone = Job.numtasks and "
                 "{} and {} and current and "
                 "rcode = 0".format(SHOW, epi, job_tq, task_tq))

    tq_columns = ['Job.title', 'jid', 'elapsedreal',
                  'Blade.numcpu', 'Blade.profile',
                  'Blade.name', 'Blade.numslots',
                  'rss', 'Job.owner', 'Task.title']

    invo_data = tq.invocations(tq_search,
                               columns=tq_columns,
                               sortby=['jid', 'tid'],
                               limit=0,
                               archive=True)

    # Grab all the jids in the old and new data
    old_jids = list(set(invo['jid'] for invo in epi_data))
    new_jids = list(set(invo['jid'] for invo in invo_data))

    # If new jids are in the old jids we save them to a list
    duplicate_jids = set()
    for jid in new_jids:
        if jid in old_jids:
            duplicate_jids.add(jid)
    duplicate_jids_list = list(duplicate_jids)

    # We will then add the EpiShotsStats comment to those jobs as
    # they should of already had that commment added
    if duplicate_jids_list:
        jids_query = ' '.join(str(e) for e in duplicate_jids_list)
        fix_jobs = tq.jobs("jid in [{}] and comment not like EpiShotStats"
                           "".format(jids_query),
                           columns=['deletetime', 'comment', 'jid'],
                           limit=0, archive=True)
        add_comment(fix_jobs)
        # Force script to error so we don't add duplicate data.
        sys.exit("Please Retry.\nDuplicate Jids: {}"
                 "".format(duplicate_jids_list))

    # Merging old data and new data into a new list
    all_data = epi_data + invo_data
    if invo_data:

        # Write the merged data to the file.
        with open(epi_stats_log, 'w') as f_obj:
            f_obj.write(json.dumps(all_data))

        # Now that we've wrote the new data, we need to add a comment
        # to those josb so we don't add them again.
        new_jids = list(set(invo['jid'] for invo in invo_data))
        jids_query = ' '.join(str(e) for e in new_jids)
        new_jobs = tq.jobs("jid in [{}]".format(jids_query),
                           columns=['deletetime', 'comment', 'jid'],
                           limit=0, archive=True)
        add_comment(new_jobs)

        # Double Check the jobs, if comment isn't there, try add it again.
        check_jobs = tq.jobs("jid in [{}] and comment not like EpiShotStats"
                             "".format(jids_query),
                             columns=['deletetime', 'comment', 'jid'],
                             limit=0, archive=True)
        add_comment(check_jobs)

    return all_data

def lgt_breakdown(dept_data):

    seq_dict = {'seq_list': []}
    for invo in dept_data:
        shot = jtu.get_shot(invo['Job.title'])
        seq = jtu.get_sequence(invo['Job.title'])

        # Setting up dictionary
        if seq not in seq_dict.keys():
            seq_dict[seq] = {'shot_list': []}
            seq_dict['seq_list'].append(seq)
        if shot not in seq_dict[seq].keys():
            seq_dict[seq]['shot_list'].append(shot)
            seq_dict[seq][shot] = {'frames': 0,
                                   'fml': 0, 'ffr': 0,
                                   'artist': [],
                                   'passes': set(),
                                   'mrss': 0, 'ctpt': [], 'ctpf': 0,
                                   'job_dict': {},
                                   'pass_dict': {}}

        if invo['jid'] not in seq_dict[seq][shot]['job_dict'].keys():
            seq_dict[seq][shot]['job_dict'][invo['jid']] = {
                'frames': 1,
                'job_type': "FML",
                'pass': jtu.get_passname(invo['Job.title']),
                'core_runtime': [rt.get_core_time(invo)],
                'memory': [invo['rss']],
                'artist': invo['Job.owner']}
        else:
            job_dict = seq_dict[seq][shot]['job_dict'][invo['jid']]
            job_dict['core_runtime'].append(rt.get_core_time(invo))
            job_dict['memory'].append(invo['rss'])
            job_dict['frames'] += 1

    return seq_dict


def get_shot_stats(seq_dict, seq, shot, shot_asset_dict):

    for job in seq_dict[seq][shot]['job_dict']:
        jdict = seq_dict[seq][shot]['job_dict'][job]
        # Core time per frame and Max RSS memory per job.
        jdict['ctpf'] = sum(jdict['core_runtime']) / jdict['frames']
        jdict['mrss'] = max(jdict['memory'])

        # Counting FML and FFR jobs in each shot
        if jdict['frames'] > 3:
            seq_dict[seq][shot]['ffr'] += 1
        else:
            seq_dict[seq][shot]['fml'] += 1
        # Max Frame count in shot
        if jdict['frames'] > seq_dict[seq][shot]['frames']:
            seq_dict[seq][shot]['frames'] = jdict['frames']
        # Max Memory of shot
        if jdict['mrss'] > seq_dict[seq][shot]['mrss']:
            seq_dict[seq][shot]['mrss'] = jdict['mrss']
        # Passes in each shot
        seq_dict[seq][shot]['passes'].add(jdict['pass'])
        # Also creating a dict to add average pass times
        if jdict['pass'] not in seq_dict[seq][shot]['pass_dict'].keys():
            seq_dict[seq][shot]['pass_dict'][jdict['pass']] = []
        seq_dict[seq][shot]['pass_dict'][jdict['pass']].append(jdict['ctpf'])
        # Adding TPT to shot list
        seq_dict[seq][shot]['ctpt'].append(jdict['ctpf'])
        # Adding artist to shot list
        seq_dict[seq][shot]['artist'].append(jdict['artist'])

    # TPF accross the shot. Getting avg per pass then adding passes
    for shot_pass in seq_dict[seq][shot]['passes']:
        shot_pass_ctpf = []
        for job in seq_dict[seq][shot]['job_dict']:
            jid_dict = seq_dict[seq][shot]['job_dict'][job]
            if shot_pass == jid_dict['pass']:
                shot_pass_ctpf.append(jid_dict['ctpf'])
        pass_avg_ctpf = sum(shot_pass_ctpf) / len(shot_pass_ctpf)
        seq_dict[seq][shot]['ctpf'] += pass_avg_ctpf
    seq_dict[seq][shot]['passes'] = len(list(seq_dict[seq][shot]['passes']))

    # Shot Time Per Task
    seq_dict[seq][shot]['ctpt'] = (
        sum(seq_dict[seq][shot]['ctpt']) /
        len(seq_dict[seq][shot]['ctpt']))
    # Shot Total Core
    seq_dict[seq][shot]['chps'] = (
        seq_dict[seq][shot]['ctpf'] *
        seq_dict[seq][shot]['frames'])
    # Artist with most jobs in the shot
    seq_dict[seq][shot]['artist'] = (
        max(set(seq_dict[seq][shot]['artist']),
            key=seq_dict[seq][shot]['artist'].count))
    # Shot Iterations
    seq_dict[seq][shot]['iter'] = (
        seq_dict[seq][shot]['ffr'] / float(seq_dict[seq][shot]['passes']))
    # Avg Time Per Pass in the shot
    for shot_pass in seq_dict[seq][shot]['pass_dict']:
        seq_dict[seq][shot]['pass_dict'][shot_pass] = (
            sum(seq_dict[seq][shot]['pass_dict'][shot_pass]) /
            len(seq_dict[seq][shot]['pass_dict'][shot_pass]))
    # Number of assets in the shot
    # seq_dict[seq][shot]['num_assets'] = shot_asset_dict[shot]['num_assets']

    return seq_dict

def get_seq_stats(seq_dict, seq):

    seq_dict[seq]['shots'] = 0
    seq_dict[seq]['frames'] = 0
    seq_dict[seq]['fml'] = []
    seq_dict[seq]['ffr'] = []
    seq_dict[seq]['passes'] = []
    seq_dict[seq]['pass_dict'] = {}
    seq_dict[seq]['iter'] = 0
    seq_dict[seq]['mrss'] = []
    seq_dict[seq]['ctpf'] = []
    seq_dict[seq]['ctpt'] = []
    ffr_chps = []
    fml_chps = []
    # Looping through each shot in the seqeunce to get shot avg.
    for shot in seq_dict[seq]['shot_list']:
        sdict = seq_dict[seq][shot]
        seq_dict[seq]['shots'] += 1
        seq_dict[seq]['frames'] += sdict['frames']
        seq_dict[seq]['fml'].append(sdict['fml'])
        seq_dict[seq]['ffr'].append(sdict['ffr'])
        seq_dict[seq]['passes'].append(sdict['passes'])
        seq_dict[seq]['ctpf'].append(sdict['ctpf'])
        seq_dict[seq]['ctpt'].append(sdict['ctpt'])
        seq_dict[seq]['mrss'].append(sdict['mrss'])
        if sdict['frames'] > 3:
            ffr_chps.append(sdict['chps'])
        else:
            fml_chps.append(sdict['chps'])
        for shot_pass in sdict['pass_dict']:
            if shot_pass not in seq_dict[seq]['pass_dict'].keys():
                seq_dict[seq]['pass_dict'][shot_pass] = []
            seq_dict[seq]['pass_dict'][shot_pass].append(sdict['pass_dict'][shot_pass])

    # Seq Avg FML jobs per shot
    seq_dict[seq]['fml'] = (sum(seq_dict[seq]['fml']) /
                            float(len(seq_dict[seq]['fml'])))
    # Seq Avg FFR jobs per shot
    seq_dict[seq]['ffr'] = (sum(seq_dict[seq]['ffr']) /
                            float(len(seq_dict[seq]['ffr'])))
    # Seq Avg Passes per shot
    seq_dict[seq]['passes'] = (sum(seq_dict[seq]['passes']) /
                               float(len(seq_dict[seq]['passes'])))
    # Seq Avg Iterations per shot
    seq_dict[seq]['iter'] = (seq_dict[seq]['ffr'] /
                             float(seq_dict[seq]['passes']))
    # Seq Avg Time Per Frame per shot
    seq_dict[seq]['ctpf'] = (sum(seq_dict[seq]['ctpf']) /
                             float(len(seq_dict[seq]['ctpf'])))
    # Seq Avg Time Per Task per shot
    seq_dict[seq]['ctpt'] = (sum(seq_dict[seq]['ctpt']) /
                             float(len(seq_dict[seq]['ctpt'])))
    # Seq Avg Core Hours per shot
    if not ffr_chps:
        ffr_chps = fml_chps
    seq_dict[seq]['chps'] = (sum(ffr_chps) /
                             float(len(ffr_chps)))
    # Seq Avg Max RSS per shot
    seq_dict[seq]['mrss'] = (sum(seq_dict[seq]['mrss']) /
                             float(len(seq_dict[seq]['mrss'])))
    # Avg Time Per Pass in the seq
    for shot_pass in seq_dict[seq]['pass_dict']:
        seq_dict[seq]['pass_dict'][shot_pass] = (
            sum(seq_dict[seq]['pass_dict'][shot_pass]) /
            len(seq_dict[seq]['pass_dict'][shot_pass]))

    return seq_dict

def get_epi_stats(seq_dict):

    seq_dict['shots'] = 0
    seq_dict['frames'] = 0
    seq_dict['fml'] = []
    seq_dict['ffr'] = []
    seq_dict['passes'] = []
    seq_dict['iter'] = 0
    seq_dict['mrss'] = []
    seq_dict['ctpf'] = []
    seq_dict['ctpt'] = []
    ffr_chps = []
    fml_chps = []
    # Looping through each shot in the episode to get shot avg.
    for seq in seq_dict['seq_list']:
        for shot in seq_dict[seq]['shot_list']:
            sdict = seq_dict[seq][shot]
            seq_dict['shots'] += 1
            seq_dict['frames'] += sdict['frames']
            seq_dict['fml'].append(sdict['fml'])
            seq_dict['ffr'].append(sdict['ffr'])
            seq_dict['passes'].append(sdict['passes'])
            seq_dict['ctpf'].append(sdict['ctpf'])
            seq_dict['ctpt'].append(sdict['ctpt'])
            seq_dict['mrss'].append(sdict['mrss'])
            if sdict['frames'] > 3:
                ffr_chps.append(sdict['chps'])
            else:
                fml_chps.append(sdict['chps'])

    # Epi Avg FML jobs per shot
    seq_dict['fml'] = (sum(seq_dict['fml']) /
                       float(len(seq_dict['fml'])))
    # Epi Avg FFR jobs per shot
    seq_dict['ffr'] = (sum(seq_dict['ffr']) /
                       float(len(seq_dict['ffr'])))
    # Epi Avg Passes per shot
    seq_dict['passes'] = (sum(seq_dict['passes']) /
                          float(len(seq_dict['passes'])))
    # Epi Avg Iterations per shot
    seq_dict['iter'] = (seq_dict['ffr'] /
                        float(seq_dict['passes']))
    # Epi Avg Time Per Frame per shot
    seq_dict['ctpf'] = (sum(seq_dict['ctpf']) /
                        float(len(seq_dict['ctpf'])))
    # Epi Avg Time Per Task per shot
    seq_dict['ctpt'] = (sum(seq_dict['ctpt']) /
                        float(len(seq_dict['ctpt'])))
    # Epi Avg Core Hours per shot
    if not ffr_chps:
        ffr_chps = fml_chps
    seq_dict['chps'] = (sum(ffr_chps) /
                        float(len(ffr_chps)))
    # Epi Avg Max RSS per shot
    seq_dict['mrss'] = (sum(seq_dict['mrss']) /
                        float(len(seq_dict['mrss'])))

    return seq_dict


def build_stats(lgt_data, shot_asset_dict):

    seq_dict = lgt_breakdown(lgt_data)

    for seq in seq_dict['seq_list']:
        # Creating shot stats
        for shot in seq_dict[seq]['shot_list']:
            seq_dict = get_shot_stats(seq_dict, seq, shot, shot_asset_dict)
        # Creating Sequence Stats
        seq_dict = get_seq_stats(seq_dict, seq)
    # Creating Episode Stats
    epi_dict = get_epi_stats(seq_dict)

    return epi_dict

def write_stats_to_json(breakdown, stats_dict):

    show_json = ("/sw/pipeline/rendering/renderfarm-reporting"
                 "/tq_data/epi_shot_stats/"
                 "{0}/show_stats.json"
                 "").format(SHOW)

    with open(show_json) as j_obj:
        show_stats = json.load(j_obj)

    for epi in stats_dict:
        if breakdown == "epi":
            if epi not in show_stats["episodes"].keys():
                show_stats["episodes"][epi] = {"tpf": 0, "tpt": 0, "chps": 0}
            show_stats["episodes"][epi]['tpf'] = round(stats_dict[epi]['ctpf'], 2)
            show_stats["episodes"][epi]['tpt'] = round(stats_dict[epi]['ctpt'], 2)
            show_stats["episodes"][epi]['chps'] = round(stats_dict[epi]['chps'], 2)
        if breakdown == "seq":
            for seq in stats_dict[epi]['seq_list']:
                if seq not in show_stats["sequences"].keys():
                    show_stats["sequences"][seq] = {"tpf": 0, "tpt": 0,
                                                    "chps": 0, "passes": {}}
                show_stats["sequences"][seq]['tpf'] = round(stats_dict[epi][seq]['ctpf'], 2)
                show_stats["sequences"][seq]['tpt'] = round(stats_dict[epi][seq]['ctpt'], 2)
                show_stats["sequences"][seq]['chps'] = round(stats_dict[epi][seq]['chps'], 2)
                # Average core time per pass on a sequence
                if 'passes' not in show_stats["sequences"][seq].keys():
                    show_stats["sequences"][seq]['passes'] = {}
                for seq_pass in stats_dict[epi][seq]['pass_dict']:
                    show_stats["sequences"][seq]['passes'][seq_pass] = round(stats_dict[epi][seq]['pass_dict'][seq_pass], 2)


    with open(show_json, 'w') as f_obj:
        json.dump(show_stats, f_obj, indent=4)


def __main__():

    # Connect to the SG API.
    sg_connection = sg_utils.get_shotgun(SHOW)[1]
    # Get the production order of the episodes on a show
    prod_order = get_production_order(sg_connection)
    # Get the assets that are assigned to a shot.
    shot_asset_dict = get_shot_assets(sg_connection)

    input_epis = [""]
    if DAILY_EPISODES or ALL_EPISODES:
        input_epis = get_episodes_rendered()
    else:
        if EPISODE:
            input_epis = [str(s) for s in EPISODE.split(',')]
    # Sorting the episodes
    input_epis.sort()

    stats_dict = {}
    for episode in input_epis:

        lgt_data = get_episode_data(episode, "LGT")
        if not lgt_data:
            continue

        # Putting stats together
        stats_dict[episode] = build_stats(lgt_data, shot_asset_dict)


    # Writing Stats
    for breakdown in BREAKDOWNS:
        datadog_print(breakdown, stats_dict, prod_order)
        if breakdown == 'epi' or breakdown == 'seq':
            # Write main stats to json file to easily grab from other scripts
            write_stats_to_json(breakdown, stats_dict)


__main__()

...
if SHOW == "LMA":
    ...
    IGNORE_EPI = ['E901', 'E991', 'E995', 'M002', 'S001', 'S002', 'S003',
                  'S004', 'S005', 'S006', 'S007', 'S008', 'S009', 'T001',
                  'T006', 'T007']
...
def get_episodes_rendered():
    ...
    jobs_rendered = tq.jobs("done and projects like {0} and title like LGT.v "
                            "and title like katana and title like {1} {2}"
...
def csv_print(stats_dict, prod_order):

    print ("Episode\tShots\tFrames\t"
           "FFR Jobs\tFML Jobs\tPasses\tIterations\t"
           "TPF(36c)\tTPT(36c)\tCore Hours Per Shot\t"
           "RSS\tANM Jobs\tANM CHPS\tAsset Count")
    ...
    for epi in stats_dict:
        ...
        print ("{}\t{:,}\t{:,}\t"
               ...
               "{:,.2f}\t"
               "{:,.1f}\t{:,.1f}\t"
               "").format(epi, epd['shots'], epd['frames'],
                          epd['ffr'], epd['fml'], epd['passes'], epd['iter'],
                          (epd['ctpf'] / 36) / 3600, (epd['ctpt'] / 36) / 3600,
                          # rt.time_format_secs(epd['ctpf'] / 36),
                          # rt.time_format_secs(epd['ctpt'] / 36),
                          (epd['chps'] / 3600.0), epd['mrss'],
                          epd['anm_jobs'],
                          (epd['anm_chps'] / 3600.0), epd['num_assets'])
...
def datadog_print(breakdown, stats_dict, prod_order):
    ...
    def create_datadog_dict(sdict):
        ...
        out_dict = {
            ...
            'shots': sdict['shots'],
            'frames': sdict['frames'],
            'ffr_jobs': round(sdict['ffr'], 2),
            'fml_jobs': round(sdict['fml'], 2),
            'passes': round(sdict['passes'], 2),
            'iterations': round(sdict['iter'], 2),
            'ctpt_stand': int(sdict['ctpt'] / 36),
            'ctpf_stand': int(sdict['ctpf'] / 36),
            'chps': round((sdict['chps'] / 3600.0), 2),
            'anm_jobs': round(sdict['anm_jobs'], 2),
            'anm_chps': round((sdict['anm_chps'] / 3600.0), 2),
            'anm_tpf': int(sdict['anm_tpf'] / 36),
            'anm_frames': sdict['anm_frames'], # TESTING
            'max_rss': round(sdict['mrss'], 2),
        ...
        if breakdown == 'epi':
            ...
            for seq in stats_dict[epi]['seq_list']:
                for shot in stats_dict[epi][seq]['shot_list']:
                    ...
                    out_dict['shot'] = shot
                    out_dict['prod_order'] = p_order
                    out_dict['artist'] = stats_dict[epi][seq][shot]['artist']
                    out_dict['shot_tag'] = ''
                    if epi in KEYSHOTS[SHOW]:
                        out_dict['shot_tag'] = 'KeyShot'
                    elif epi in PARENTSHOTS[SHOW]:
                        out_dict['shot_tag'] = 'ParentShot'
                    log_file.write("{}\n".format(json.dumps(out_dict)))
...
def lgt_breakdown(dept_data):
    ...
    for invo in dept_data:
        ...
        if seq not in seq_dict.keys():
            seq_dict[seq] = {'shot_list': []}
            seq_dict['seq_list'].append(seq)
        if shot not in seq_dict[seq].keys():
            seq_dict[seq]['shot_list'].append(shot)
            seq_dict[seq][shot] = {'frames': 0,
                                   'fml': 0, 'ffr': 0,
                                   'artist': [],
                                   'passes': set(),
                                   'mrss': 0, 'ctpt': [], 'ctpf': 0,
                                   'anm_jobs': 0, 'anm_chps': [],
                                   'anm_frames': 0, 'anm_tpf': [],
                                   'job_dict': {}, 'anm_job_dict': {}}

        if invo['jid'] not in seq_dict[seq][shot]['job_dict'].keys():
            seq_dict[seq][shot]['job_dict'][invo['jid']] = {
                'frames': 1,
                'job_type': "FML",
                'pass': jtu.get_passname(invo['Job.title']),
                'core_runtime': [rt.get_core_time(invo)],
                'memory': [invo['rss']],
                'artist': invo['Job.owner']}
        else:
            job_dict = seq_dict[seq][shot]['job_dict'][invo['jid']]
            job_dict['core_runtime'].append(rt.get_core_time(invo))
            job_dict['memory'].append(invo['rss'])
            job_dict['frames'] += 1
...
def anm_breakdown(dept_data, lgt_seq_dict):
    ...
    for invo in dept_data:
        ...
        # Only adding ANM stats to shots that have LGT data
        if not lgt_seq_dict.get(seq, 0):
            continue
        if not lgt_seq_dict[seq].get(shot, 0):
            continue

        # Adding up the core time per task
        if invo['jid'] not in lgt_seq_dict[seq][shot]['anm_job_dict'].keys():
            lgt_seq_dict[seq][shot]['anm_job_dict'][invo['jid']] = {
                'core_runtime': ctime, 'frames': frames}
        else:
            lgt_seq_dict[seq][shot]['anm_job_dict'][invo['jid']]['core_runtime'] += ctime
            lgt_seq_dict[seq][shot]['anm_job_dict'][invo['jid']]['frames'] += frames

    for seq in lgt_seq_dict['seq_list']:
        for shot in lgt_seq_dict[seq]['shot_list']:
            for job in lgt_seq_dict[seq][shot]['anm_job_dict']:
                # Counting number of ANM jobs
                lgt_seq_dict[seq][shot]['anm_jobs'] += 1
                jdict = lgt_seq_dict[seq][shot]['anm_job_dict'][job]
                lgt_seq_dict[seq][shot]['anm_chps'].append(jdict['core_runtime'])
                # ANM Frame count in shot
                if lgt_seq_dict[seq][shot]['anm_frames'] < jdict['frames']:
                    lgt_seq_dict[seq][shot]['anm_frames'] = jdict['frames']
                # Adding TPF of job to TPF shot list
                lgt_seq_dict[seq][shot]['anm_tpf'].append(
                    jdict['core_runtime'] / jdict['frames'])

            # Getting the average Core Hours Per ANM Shot
            if lgt_seq_dict[seq][shot]['anm_chps']:
                lgt_seq_dict[seq][shot]['anm_chps'] = (
                    sum(lgt_seq_dict[seq][shot]['anm_chps']) /
                    len(lgt_seq_dict[seq][shot]['anm_chps']))
            else:
                lgt_seq_dict[seq][shot]['anm_chps'] = 0
            # Getting the average TPF Per ANM Shot
            if lgt_seq_dict[seq][shot]['anm_tpf']:
                lgt_seq_dict[seq][shot]['anm_tpf'] = (
                    sum(lgt_seq_dict[seq][shot]['anm_tpf']) /
                    len(lgt_seq_dict[seq][shot]['anm_tpf']))
            else:
                lgt_seq_dict[seq][shot]['anm_tpf'] = 0
...
def get_shot_stats(seq_dict, seq, shot, shot_asset_dict):
    ...
    for job in seq_dict[seq][shot]['job_dict']:
        ...
        jdict['ctpf'] = sum(jdict['core_runtime']) / jdict['frames']
        jdict['mrss'] = max(jdict['memory'])

        # Counting FML and FFR jobs in each shot
        if jdict['frames'] > 3:
            seq_dict[seq][shot]['ffr'] += 1
        else:
            seq_dict[seq][shot]['fml'] += 1
        # Max Frame count in shot
        if jdict['frames'] > seq_dict[seq][shot]['frames']:
            seq_dict[seq][shot]['frames'] = jdict['frames']
        # Max Memory of shot
        if jdict['mrss'] > seq_dict[seq][shot]['mrss']:
            seq_dict[seq][shot]['mrss'] = jdict['mrss']
        # Passes in each shot
        seq_dict[seq][shot]['passes'].add(jdict['pass'])
        # Adding TPT to shot list
        seq_dict[seq][shot]['ctpt'].append(jdict['ctpf'])
        # Adding artist to shot list
        seq_dict[seq][shot]['artist'].append(jdict['artist'])

    # TPF accross the shot. Getting avg per pass then adding passes
    for shot_pass in seq_dict[seq][shot]['passes']:
        shot_pass_ctpf = []
        for job in seq_dict[seq][shot]['job_dict']:
            jid_dict = seq_dict[seq][shot]['job_dict'][job]
            if shot_pass == jid_dict['pass']:
                shot_pass_ctpf.append(jid_dict['ctpf'])
        pass_avg_ctpf = sum(shot_pass_ctpf) / len(shot_pass_ctpf)
        seq_dict[seq][shot]['ctpf'] += pass_avg_ctpf
    seq_dict[seq][shot]['passes'] = len(list(seq_dict[seq][shot]['passes']))

    # Shot Time Per Task
    seq_dict[seq][shot]['ctpt'] = (
        sum(seq_dict[seq][shot]['ctpt']) /
        len(seq_dict[seq][shot]['ctpt']))
    # Shot Total Core
    seq_dict[seq][shot]['chps'] = (
        seq_dict[seq][shot]['ctpf'] *
        seq_dict[seq][shot]['frames'])
    # Artist with most jobs in the shot
    seq_dict[seq][shot]['artist'] = (
        max(set(seq_dict[seq][shot]['artist']),
            key=seq_dict[seq][shot]['artist'].count))
    # Shot Iterations
    seq_dict[seq][shot]['iter'] = (
        seq_dict[seq][shot]['ffr'] / float(seq_dict[seq][shot]['passes']))
...
def get_seq_stats(seq_dict, seq):
    ...
    for shot in seq_dict[seq]['shot_list']:
        ...
        seq_dict[seq]['frames'] += sdict['frames']
        seq_dict[seq]['fml'].append(sdict['fml'])
        seq_dict[seq]['ffr'].append(sdict['ffr'])
        seq_dict[seq]['passes'].append(sdict['passes'])
        seq_dict[seq]['ctpf'].append(sdict['ctpf'])
        seq_dict[seq]['ctpt'].append(sdict['ctpt'])
        seq_dict[seq]['mrss'].append(sdict['mrss'])
        ...
        if sdict['frames'] > 3:
            ffr_chps.append(sdict['chps'])
        else:
            fml_chps.append(sdict['chps'])

    # Seq Avg FML jobs per shot
    seq_dict[seq]['fml'] = (sum(seq_dict[seq]['fml']) /
                            float(len(seq_dict[seq]['fml'])))
    # Seq Avg FFR jobs per shot
    seq_dict[seq]['ffr'] = (sum(seq_dict[seq]['ffr']) /
                            float(len(seq_dict[seq]['ffr'])))
    # Seq Avg Passes per shot
    seq_dict[seq]['passes'] = (sum(seq_dict[seq]['passes']) /
                               float(len(seq_dict[seq]['passes'])))
    # Seq Avg Iterations per shot
    seq_dict[seq]['iter'] = (seq_dict[seq]['ffr'] /
                             float(seq_dict[seq]['passes']))
    # Seq Avg Time Per Frame per shot
    seq_dict[seq]['ctpf'] = (sum(seq_dict[seq]['ctpf']) /
                             float(len(seq_dict[seq]['ctpf'])))
    # Seq Avg Time Per Task per shot
    seq_dict[seq]['ctpt'] = (sum(seq_dict[seq]['ctpt']) /
                             float(len(seq_dict[seq]['ctpt'])))
    # Seq Avg Core Hours per shot
    if not ffr_chps:
        ffr_chps = fml_chps
    seq_dict[seq]['chps'] = (sum(ffr_chps) /
                             float(len(ffr_chps)))
    # Seq Avg Max RSS per shot
    seq_dict[seq]['mrss'] = (sum(seq_dict[seq]['mrss']) /
                             float(len(seq_dict[seq]['mrss'])))
...
def get_epi_stats(seq_dict):
    ...
    for seq in seq_dict['seq_list']:
        for shot in seq_dict[seq]['shot_list']:
            sdict = seq_dict[seq][shot]
            seq_dict['shots'] += 1
            seq_dict['frames'] += sdict['frames']
            seq_dict['fml'].append(sdict['fml'])
            seq_dict['ffr'].append(sdict['ffr'])
            seq_dict['passes'].append(sdict['passes'])
            seq_dict['ctpf'].append(sdict['ctpf'])
            seq_dict['ctpt'].append(sdict['ctpt'])
            seq_dict['mrss'].append(sdict['mrss'])
    ...
    # Epi Avg FML jobs per shot
    seq_dict['fml'] = (sum(seq_dict['fml']) /
                       float(len(seq_dict['fml'])))
    # Epi Avg FFR jobs per shot
    seq_dict['ffr'] = (sum(seq_dict['ffr']) /
                       float(len(seq_dict['ffr'])))
    # Epi Avg Passes per shot
    seq_dict['passes'] = (sum(seq_dict['passes']) /
                          float(len(seq_dict['passes'])))
    # Epi Avg Iterations per shot
    seq_dict['iter'] = (seq_dict['ffr'] /
                        float(seq_dict['passes']))
    # Epi Avg Time Per Frame per shot
    seq_dict['ctpf'] = (sum(seq_dict['ctpf']) /
                        float(len(seq_dict['ctpf'])))
    # Epi Avg Time Per Task per shot
    seq_dict['ctpt'] = (sum(seq_dict['ctpt']) /
                        float(len(seq_dict['ctpt'])))
    # Epi Avg Core Hours per shot
    if not ffr_chps:
        ffr_chps = fml_chps
    seq_dict['chps'] = (sum(ffr_chps) /
                        float(len(ffr_chps)))
    # Epi Avg Max RSS per shot
    seq_dict['mrss'] = (sum(seq_dict['mrss']) /
                        float(len(seq_dict['mrss'])))
...
def build_stats(lgt_data, anm_data, shot_asset_dict):

    lgt_seq_dict = lgt_breakdown(lgt_data)
    seq_dict = anm_breakdown(anm_data, lgt_seq_dict)
...
def write_stats_to_json(breakdown, stats_dict):

    show_json = ("/sw/pipeline/rendering/renderfarm-reporting"
                 "/tq_data/epi_shot_stats/"
                 "{0}/show_stats.json"
                 "").format(SHOW)

    with open(show_json) as j_obj:
        show_stats = json.load(j_obj)

    for epi in stats_dict:
        if breakdown == "epi":
            if epi not in show_stats["episodes"].keys():
                show_stats["episodes"][epi] = {"tpf": 0, "tpt": 0, "chps": 0}
            show_stats["episodes"][epi]['tpf'] = stats_dict[epi]['ctpf']
            show_stats["episodes"][epi]['tpt'] = stats_dict[epi]['ctpt']
            show_stats["episodes"][epi]['chps'] = stats_dict[epi]['chps']
        if breakdown == "seq":
            for seq in stats_dict[epi]['seq_list']:
                if seq not in show_stats["sequences"].keys():
                    show_stats["sequences"][seq] = {"tpf": 0, "tpt": 0, "chps": 0}
                show_stats["sequences"][seq]['tpf'] = stats_dict[epi][seq]['ctpf']
                show_stats["sequences"][seq]['tpt'] = stats_dict[epi][seq]['ctpt']
                show_stats["sequences"][seq]['chps'] = stats_dict[epi][seq]['chps']


    with open(show_json, 'w') as f_obj:
        json.dump(show_stats, f_obj, indent=4)
...
def __main__():
    ...
    for episode in input_epis:
        ...
        lgt_data = get_episode_data(episode, "LGT")
        anm_data = get_episode_data(episode, "ANM")
        if not lgt_data:
            continue

        # Putting stats together
        stats_dict[episode] = build_stats(lgt_data, anm_data, shot_asset_dict)


    # Writing Stats
    for breakdown in BREAKDOWNS:
        datadog_print(breakdown, stats_dict, prod_order)
        if breakdown == 'epi' or breakdown == 'seq':
            # Write main stats to json file to easily grab from other scripts
            write_stats_to_json(breakdown, stats_dict)
        #     csv_print(breakdown, stats_dict, prod_order)
